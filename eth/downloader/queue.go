...
var (
	blockCacheMaxItems     = 4096              // Maximum number of blocks to cache before throttling the download
	blockCacheInitialItems = 1024              // Initial number of blocks to start fetching, before we know the sizes of the blocks
...
func (q *queue) deliver(id string, taskPool map[common.Hash]*types.Header,
	...
	for _, header := range request.Headers[:i] {
		// Upstream b32d20324: a stale slot must not be reconstructed into, the
		// result it points at already belongs to a different (later) fetch round.
