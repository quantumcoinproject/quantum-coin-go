...
import (
	...
	"github.com/quantumcoinproject/quantum-coin-go/eth/tracers/logger"
...
func newTestBackend(t *testing.T, n int, gspec *core.Genesis, generator func(i int, b *core.BlockGen)) *testBackend {
	...
	if count, err := chain.InsertChain(blocks); err != nil {
		t.Fatalf("block %d: failed to insert into chain: %v, count %d", n, err, count)
	}
	backend.chain = chain
	return backend
}
...
func (b *testBackend) StateAtBlock(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, checkLive bool, preferDisk bool) (*state.StateDB, error) {
	statedb, err := b.chain.StateAt(block.Root(), new(big.Int).Add(block.Number(), common.Big1))
	if err != nil {
...
func TestTraceCall(t *testing.T) {
	...
	api := NewAPI(newTestBackend(t, genBlocks, genesis, func(i int, b *core.BlockGen) {
		...
		//    value: 1000 wei
		//    fee:   0 wei
		unsignedTx := types.NewTransaction(uint64(i), accounts[1].addr, big.NewInt(1000), params.TxGas, nil, nil)
		tx, _ := types.SignTx(unsignedTx, signer, accounts[0].key)
		b.AddTx(tx)
	}))
...
func TestTraceTransaction(t *testing.T) {
	...
	if !reflect.DeepEqual(have, want) {
		t.Errorf("Result mismatch, want %v, get %v", want, have)
	}
}
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &ethapi.StateOverride{
					randomAccounts[0].addr: ethapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), params.EtherToWei(big.NewInt(100000))))},
				},
			},
...
func newAccounts(n int) (accounts Accounts) {
	for i := 0; i < n; i++ {
		...
		addr, _ := cryptobase.SigAlg.PublicKeyToAddress(&key.PublicKey)
