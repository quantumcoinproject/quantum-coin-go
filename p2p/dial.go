...
func (d *dialScheduler) startDial(task *dialTask) {
	...
	d.SetDialingMapItem(node.ID(), task)
	go func() {
		task.run(d)
		d.doneCh <- task
	}()
...
type dialTask struct {
	...
	// Upstream 758fce71f: resolve() rewrites the destination from the task
	// goroutine while the scheduler reads it, so it is held atomically.
...
// dest returns the current destination of the task.
...
func (t *dialTask) dial(d *dialScheduler, dest *enode.Node) error {
	log.Trace("dialling", "ip", dest.ID(), "port", dest.TCP())
