...
type DumpCollector interface {
	...
	OnRoot(common.Hash)
	// OnAccount is called once for each account in the trie. The address is nil
	// only when the preimage is unavailable - a nil pointer is used rather than
	// the zero address so that the genuine zero-address account is not dropped
	// (upstream bfded65ed).
