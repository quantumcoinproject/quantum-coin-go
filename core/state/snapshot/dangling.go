...
//
// Upstream 59ac229f8 (with the journal half from 0914234d1): the disk layer and
// the journalled diff layers must be inspected separately, iterating a combined
// view hides a dangle on disk behind an account resurrected in a diff layer.
...
func checkDanglingDiskStorage(chaindb ethdb.KeyValueStore) error {
	var (
		...
		start      = time.Now()
		lastKey    []byte
		it         = newKeyLengthIterator(chaindb.NewIterator(rawdb.SnapshotStoragePrefix, nil), len(rawdb.SnapshotStoragePrefix)+2*common.HashLength)
	)
	...
	for it.Next() {
		k := it.Key()
		accKey := k[len(rawdb.SnapshotStoragePrefix) : len(rawdb.SnapshotStoragePrefix)+common.HashLength]
...
func checkDanglingMemStorage(db ethdb.KeyValueStore) error {
	...
	// It can happen that the node crashes without persisting the latest
	// diff journal.
...
// checkDanglingJournalStorage reads the sections of a snapshot journal one by
// one and verifies that every storage entry has an accompanying account entry
// in the very same diff layer.
