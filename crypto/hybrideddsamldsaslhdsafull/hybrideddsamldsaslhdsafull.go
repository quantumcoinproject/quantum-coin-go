...
func (s HybridEddsaMldsaSlhdsaFullSig) PublicKeyAndSignatureFromCombinedSignatureWithContext(digestHash []byte, sig []byte, context []byte) (signature []byte, pubKey []byte, digest []byte, err error) {
	signature, pubKey, err = common.ExtractTwoParts(sig)
	if err != nil {
		return nil, nil, nil, err
	}

	if context != nil && len(context) > 0 && context[0] == byte(crypto.MLDSA_ED25519_SLHDSA_FULL_ID) {
		newDigestHash := crypto.Keccak256(digestHash, context)
...
func (s HybridEddsaMldsaSlhdsaFullSig) PreExpansionSeedSize() int {
	return 0
}

func (s HybridEddsaMldsaSlhdsaFullSig) GenerateKeyFromPreExpansionSeed(preExpansionSeed []byte) (*signaturealgorithm.PrivateKey, error) {
