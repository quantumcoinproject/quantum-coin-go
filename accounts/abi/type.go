...
func NewType(t string, internalType string, components []ArgumentMarshaling) (typ Type, err error) {
	...
	case "bytes":
		if varSize == 0 {
			...
			// Upstream 8578eb2fe: bytesN is only defined for 0 < N <= 32.
			if varSize > 32 {
				return Type{}, fmt.Errorf("unsupported arg type: %s", t)
			}
			typ.T = FixedBytesTy
	...
	case "tuple":
		...
		for idx, c := range components {
			...
			// Upstream af02e9792: reflect.StructOf panics on a malformed field
			// name, which is reachable from untrusted ABI JSON.
