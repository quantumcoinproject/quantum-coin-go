...
// requests. Upstream f3314bb6d: the whole response batch is delivered at once, so the
// client can tell when the server did not answer every call in a batch.
...
func initClient(conn ServerCodec, idgen func() ID, services *serviceRegistry, batchItemLimit, batchResponseMaxSize int) *Client {
...
func (c *Client) CallContext(ctx context.Context, result interface{}, method string, args ...interface{}) error {
	...
	case len(resp.Result) == 0:
		return ErrNoResult
	default:
		// Upstream 1db978ca6: unmarshal into result itself, not into a pointer to the
		// interface, so that a JSON 'null' result lands in the caller's value.
		if result == nil {
			return nil
		}
...
func (c *Client) BatchCallContext(ctx context.Context, b []BatchElem) error {
	...
	for i, elem := range b {
		...
		msgs[i] = msg
		op.ids[i] = msg.ID
		// Upstream 53b94f135: index the requests by ID instead of rescanning the batch
		// for every response.
	...
	// Wait for the response batch. Upstream f3314bb6d: the server answers a batch with a
	// single response batch, which may be shorter than the request when a limit was hit.
	batchresp, err := op.wait(ctx, c)
	if err != nil {
		return err
	}
	...
	for _, index := range byID {
		b[index].Error = ErrMissingBatchResponse
	}
