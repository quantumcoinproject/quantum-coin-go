...
interface IStakingContract {
    ...
    //Staking V3 functions

    //Bonding: a validator bonds a depositor (validator-side exclusive, pair-keyed) before the depositor can call newDeposit
    function bondDepositor(address depositorAddress) external;
    //Rotation bond (F15): the new validator bonds an existing depositor before that depositor can changeValidator to it
    function bondDepositorForRotation(address depositorAddress) external;
    //Serves BOTH flows (same pair map)
    function isDepositorBonded(address depositorAddress, address validatorAddress) external view returns (bool);
    ...
    //Staking V3 events
    event OnBondDepositor(address indexed depositorAddress, address indexed validatorAddress);
    event OnBondDepositorForRotation(address indexed depositorAddress, address indexed newValidatorAddress);
}
...
contract StakingContract is IStakingContract {
    ...
    //StakingV3 variables - MUST stay at the end; do not reorder anything above this
    //Reentrancy lock
    bool private _locked;

    // F3: pair-keyed so competing bonds cannot overwrite/front-run a legit (depositor, validator) pair
    mapping (address => mapping (address => bool)) private _depositorBond; // depositor => validator => bonded

    // F13: validator-side exclusivity - a validator may bond at most one depositor (set true on bond, never reset)
    mapping (address => bool) private _validatorBonded; // validator => has bonded a depositor

    modifier nonReentrant() {
        require(_locked == false, "ReentrancyGuard: reentrant call");
        _locked = true;
        _;
        _locked = false;
    }

    function bondDepositor(address depositorAddress) override external nonReentrant {
        address validatorAddress = msg.sender;
        require(depositorAddress != address(0), "Invalid depositor");
        require(depositorAddress != validatorAddress, "Depositor address cannot be same as Validator address");

        // F13: validator must not have already bonded a depositor
        require(_validatorBonded[validatorAddress] == false, "Validator already bonded a depositor");

        // Validator must not already exist / have existed (as validator or depositor)
        ...
        // Depositor must not already exist / have existed (as validator or depositor)
        ...
        _validatorBonded[validatorAddress] = true;
        _depositorBond[depositorAddress][validatorAddress] = true;

        emit OnBondDepositor(depositorAddress, validatorAddress);
    }

    function bondDepositorForRotation(address depositorAddress) override external nonReentrant {
        address newValidatorAddress = msg.sender;
        require(depositorAddress != address(0), "Invalid depositor");
        require(depositorAddress != newValidatorAddress, "Depositor address cannot be same as Validator address");

        // F13 one-shot: the new validator must not have already bonded anyone
        require(_validatorBonded[newValidatorAddress] == false, "Validator already bonded a depositor");

        // New validator must be a fresh address (never a validator or depositor)
        require(_validatorExists[newValidatorAddress] == false, "Validator already exists");
        require(_validatorEverExisted[newValidatorAddress] == false, "Validator existed once");
        require(_depositorExists[newValidatorAddress] == false, "Depositor already exists as new validator once");
        require(_depositorEverExisted[newValidatorAddress] == false, "Depositor existed once as new validator");

        // Rotation flow: depositor MUST already exist (opposite of bondDepositor)
        require(_depositorExists[depositorAddress] == true, "Depositor does not exist");

        _validatorBonded[newValidatorAddress] = true;
        _depositorBond[depositorAddress][newValidatorAddress] = true;

        emit OnBondDepositorForRotation(depositorAddress, newValidatorAddress);
    }

    function isDepositorBonded(address depositorAddress, address validatorAddress) override external view returns (bool) {
        return _depositorBond[depositorAddress][validatorAddress];
    }

    function newDeposit(address validatorAddress) override external payable nonReentrant {
        ...
        require(depositorAddress != validatorAddress, "Depositor address cannot be same as Validator address");
        require(validatorAddress != address(0), "Invalid validator"); // retained (F1)

        // F1/F3: pair-keyed bond gate. The validator must have bonded this depositor for this exact validator
        // address (set via a validator-signed bondDepositor txn), which replaces the v2 zero-balance heuristic.
        // The gate is a bool defaulting to false, so an un-bonded validator (including address(0)) reverts here.
        require(_depositorBond[depositorAddress][validatorAddress], "Validator has not bonded depositor");
        ...
        // NOTE: do NOT delete _depositorBond[depositorAddress][validatorAddress] - the bond pair is intentionally retained
    ...
    function pauseValidation() override external nonReentrant {
        address depositorAddress = msg.sender;
        require(_depositorExists[depositorAddress] == true, "Depositor does not exist");
    ...
    function resumeValidation() override external nonReentrant {
        address depositorAddress = msg.sender;
        require(_depositorExists[depositorAddress] == true, "Depositor does not exist");
    ...
    function changeValidator(address newValidatorAddress) override external nonReentrant {
        ...
        require(_depositorExists[depositorAddress] == true, "Depositor does not exist");
        // F15: the new validator must have bonded this depositor for rotation (pair-keyed). The bond proves
        // control of the new validator key, replacing the removed zero-balance heuristic. The pair is NOT cleared.
        require(_depositorBond[depositorAddress][newValidatorAddress], "New validator has not bonded depositor");
    ...
    function increaseDeposit() override external payable nonReentrant {
        address depositorAddress = msg.sender;
        require(_depositorExists[depositorAddress] == true, "Depositor does not exist");
    ...
    function initiatePartialWithdrawal(uint256 amount) override external nonReentrant returns (uint256) {
        address depositorAddress = msg.sender;
        require(_depositorExists[depositorAddress] == true, "Depositor does not exist");
        require(_depositorWithdrawalRequests[depositorAddress] == 0, "Depositor withdrawal request exists");
    ...
    function completePartialWithdrawal() override external nonReentrant returns (uint256) {
        address depositorAddress = msg.sender;
        require(_depositorExists[depositorAddress] == true, "Depositor does not exist");
        require(_depositorWithdrawalRequests[depositorAddress] == 0, "Depositor withdrawal request exists");
